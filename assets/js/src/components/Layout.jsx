...
const Layout = ({ children }) => {
  ...
  // Get permissions from Redux state once
  const accessState = useSelector(state => state.access);
  
  // Filter navigation items based on user access permissions
  const navItems = allNavItems.filter(item => {
    // Always include dashboard or items without routeName requirement
    if (item.routeName === null) return true;
    
    // Check access based on the permissions already retrieved
    return selectHasAccess(accessState, item.routeName);
  });
